...
export function LocationConsentDialog({ open, onAccept }: LocationConsentDialogProps) {
  return (
    <Dialog open={open}>
      <DialogContent className="rounded-3xl max-w-[90vw] sm:max-w-md max-h-[85vh] max-h-[85dvh] p-5 sm:p-6 bg-background/95 backdrop-blur-xl border border-border shadow-2xl flex flex-col overflow-hidden">
        {/* Conteúdo com rolagem para telas menores */}
        <div className="flex-1 overflow-y-auto pr-1 space-y-4">
          <DialogHeader>
            ...
            <DialogTitle className="text-xl sm:text-2xl font-bold text-center mb-2">Permitir Localização</DialogTitle>
            ...
            <DialogDescription className="text-center text-muted-foreground leading-relaxed text-xs sm:text-sm mt-2" id="location-dialog-description">
          ...
          <div className="bg-muted/50 rounded-2xl p-3.5 sm:p-4 flex items-start gap-3 mt-2">
        ...
        {/* Botão sempre visível e acessível no rodapé */}
        <DialogFooter className="mt-4 pt-2 shrink-0">
          <Button 
            onClick={onAccept} 
            className="w-full h-12 rounded-2xl font-bold text-base sm:text-lg gradient-primary text-primary-foreground shadow-lg shadow-primary/20"
          >
            Entendi e Aceito
          </Button>
        </DialogFooter>
