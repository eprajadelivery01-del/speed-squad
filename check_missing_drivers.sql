...
-- 1) VISÃO GERAL: quantos usuários x perfis x delivery_drivers x roles existem
SELECT
  (SELECT count(*) FROM auth.users)                                   AS auth_users,
  (SELECT count(*) FROM public.profiles)                              AS profiles,
  (SELECT count(*) FROM public.delivery_drivers)                       AS delivery_drivers,
...
SELECT u.email,
       ...
       (d.id IS NOT NULL) AS tem_delivery_driver,
       (r.user_id IS NOT NULL) AS tem_role_driver,
       u.email_confirmed_at IS NOT NULL AS email_confirmado,
       u.last_sign_in_at
FROM auth.users u
LEFT JOIN public.profiles p ON p.id = u.id
LEFT JOIN public.delivery_drivers  d ON d.user_id = u.id
...
-- 10) POLÍTICAS RLS DE delivery_drivers / profiles / user_roles
-- (linhas podem estar sendo ocultadas, não apagadas)
SELECT schemaname, tablename, policyname, cmd, roles, qual
FROM pg_policies
WHERE schemaname = 'public' AND tablename IN ('delivery_drivers','profiles','user_roles')
...
-- =====================================================================
-- -- 3-fix: cria delivery_drivers faltantes para profiles marcados como entregador
-- INSERT INTO public.delivery_drivers (user_id, full_name, phone, document)
-- SELECT p.id, p.full_name, p.phone, p.document
-- FROM public.profiles p
-- LEFT JOIN public.delivery_drivers d ON d.user_id = p.id
-- WHERE d.id IS NULL
...
--
-- -- 4-fix: garante a role 'driver' para quem já tem registro em delivery_drivers
